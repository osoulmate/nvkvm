  ...
  extends JDialog
{
  ...
  public int bladeNO_CombinationKey;
  public transient PackData pack_CombinationKey;
  ...
  public transient BladeThread bladeThread = null;
  private CombinationKey(JFrame frame) {
    super(frame, true);
  }
  public CombinationKey(JFrame frame, int bladeNO_CombinationKey, PackData pack_CombinationKey) {
    this(frame);
    this.bladeNO_CombinationKey = bladeNO_CombinationKey;
    this.pack_CombinationKey = pack_CombinationKey;
    jbInit();
  }
  public CombinationKey(JDialog dialog, int bladeNO_CombinationKey, PackData pack_CombinationKey, KVMInterface kvmInterface2) {
    this.bladeNO_CombinationKey = bladeNO_CombinationKey;
    this.pack_CombinationKey = pack_CombinationKey;
    this.kvmInterface = kvmInterface2;
    this.send = new JButton(kvmInterface2.getKvmUtil().getString("send"));
    jbInit();
  }
  private void jbInit() {
    this.bladeThread = (BladeThread)this.kvmInterface.getBase().getThreadGroup().get(String.valueOf(this.bladeNO_CombinationKey));
    setSize(550, 120);
    setResizable(false);
    ...
    addWindowListener(new WindowAdapter()
        {
          ...
          {
            if (CombinationKey.this.kvmInterface.getBase().getHookNum() == 0)
            {
              CombinationKey.this.kvmInterface.getBase().setHookNum(MouseDisplacementImpl.installHook());
            }
            CombinationKey.this.kvmInterface.getKvmUtil().setiWindosFocus(1);
          }
          public void windowDeactivated(WindowEvent e) {
            CombinationKey.this.kvmInterface.getKvmUtil().setiWindosFocus(0);
            if (CombinationKey.this.kvmInterface.getBase().getHookNum() != 0) {
              MouseDisplacementImpl.removeHook(CombinationKey.this.kvmInterface.getBase().getHookNum());
              CombinationKey.this.kvmInterface.getBase().setHookNum(0);
            } 
          }
        });
    ...
    this.tPanel.add(new JLabel(this.kvmInterface.getKvmUtil().getString("custom"), 0), (Object)null);
