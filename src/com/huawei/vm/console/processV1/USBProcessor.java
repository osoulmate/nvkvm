...
public abstract class USBProcessor
{
  ...
  protected boolean bSecretVMM = false;
  protected byte[] secretKey = null;
  protected byte[] secretIV = null;
  ...
  protected static final int PACKET_OFFSET = 12;
  public USBProcessor(CommunicationSender sender) {
    ...
    this.outputBuffer = new byte[131072];
    ...
    while (!this.exitFlag && length > 0) {
      try {
        ...
        if (curElement.isData()) {
          ...
          curElement = this.dataArray.getAndRemoveFirst();
    ...
    while (!this.exitFlag && length > 0) {
      try {
        ...
        if (curElement.isData()) {
          ...
          byte[] decryData = AESHandler.aes_cbc_128_decrypt(data, this.secretKey, this.secretIV);
  ...
  public void setSecretKey(boolean bSecret, byte[] key, byte[] iv) {
